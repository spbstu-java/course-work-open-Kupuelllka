package com.example.kursach.Classes.Translator;
...
public class Translator {
    ...
    public boolean wordExists(String word) {
        if (word == null || word.trim().isEmpty()) {
            AppLogger.warning("Попытка проверить существование null или пустого слова");
            return false;
        }

        if (dictionary == null || dictionary.isEmpty()) {
            AppLogger.warning("Попытка проверить слово в незагруженном словаре");
            return false;
        }

        boolean exists = dictionary.containsKey(word.toLowerCase());
        AppLogger.fine("Проверка существования слова '" + word + "': " + exists);
        return exists;
    }
    ...
    /**
     * Переводит одно слово (без greedy matching)
     */
    public String translateWord(String word) throws Exceptions.TranslationException {
        AppLogger.info("Начало перевода слова: '" + word + "'");

        if (word == null) {
            AppLogger.warning("Передан null вместо слова для перевода");
            throw new Exceptions.InvalidInputException("Слово не может быть null");
        }

        if (word.trim().isEmpty()) {
            AppLogger.warning("Передано пустое слово для перевода");
            return word;
        }
        ...
        try {
            ...
            if (translation != null) {
                AppLogger.info("Слово переведено: '" + word + "' -> '" + translation + "'");
                return translation;
            } else {
                AppLogger.info("Слово не найдено в словаре: '" + word + "'");
                return word; // Возвращаем оригинальное слово, если перевод не найден
            }

        } catch (Exception e) {
            AppLogger.log(Level.SEVERE, "Ошибка при переводе слова: " + word, e);
            throw new Exceptions.TranslationException("Ошибка перевода слова: " + e.getMessage(), e);
        }
    }
